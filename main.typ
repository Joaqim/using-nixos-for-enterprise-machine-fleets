...
#show: ieee.with(
  title: [Using NixOS for Enterprise Machine Fleets ],
  abstract: [
    The availability of @llm-assisted coding tools raises a specific risk for system administration: an operator can use an @llm:short to make ad hoc changes directly over @root @ssh access to a live machine - "@vibe-coding" a deployment into existence with no record of what changed or why. We argue for the alternative: writing deployments locally in a declarative, human-readable syntax, where @llm:short assistance operates on @vcs:long rather than production state. Using @nixos, we present a project structure that keeps machine development and live debugging access clearly separated,
  ],
  authors: (
    (
      name: "Joaqim Planstedt",
      department: [IT],
      organization: [/*Shipify AB*/],
      location: [Gothenburg, Sweden],
      email: "mail@joaqim.xyz"
    ),
  ),
  index-terms: ("IT","LLM","NixOS","Enterprise software"),
  bibliography: bibliography("refs.bib"),
...
#block(breakable: false)[
  ...
  == Dendritic pattern
  The @dendritic-pattern composes a @flake from many small @flake-parts @module:pl with no manual import list.
